    ...
    <div class="card">
        <div class="card-body">
            ...
            <div style="padding-top: 20px">
                {!! $slides->appends(compact('items'))->links()!!}
            </div>
            @endsection
            @section('form-add')
                <div id="image"></div>
                ...
                <script>
                    $(document).ready(function () {
                        ...
                        $(document).on('click', '.add-image', function () {
                            $.get('admin/slide/create', function (data) {
                                $("#image").html(data);
                        ...
                        $(document).on('click', '.edit-image', function () {
                            ...
                            $.get('admin/slide/' + image_id + '/edit', function (data) {
                                $("#image").html(data);
                        ...
                        if ($("#addForm").length > 0) {
                            ...
                            $.ajax({
                                ...
                                success: function (data) {
                                    var dataItem = '<tr id="id_' + data.id + '"><td>' + data.id + '</td><td><a href="javascript:void(0)" class="show-image" data-id ="' + data.id + '"><img id="image_' + data.id + '"  src = "images/' + data.image + '" alt ="" style="height:50px;width: 50px" class="img-responsive" />' + '</td><td>' + data.created_at + '</td>';
                    ...
                    $('body').on('click', '#delete-image', function (event) {
                        ...
                        if (confirm("Are you sure want to delete this field !")) {
                            $.ajax({
                                type: "GET",
                                url: "admin/slide/destroy/" + image_id,
                                contentType: false,
                                processData: false,
                                success: function (data) {
                                    $("#id_" + image_id).remove();
