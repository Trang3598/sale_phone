$('#search-data').change(function (event) {
    ...
    $.ajax({
        ...
        success: function (data) {
            var count = Object.keys(data.products.data).length;
            for (let i = 0; i < count; i++) {
                array[i] = (data.products.data[i].name_phone);
            }
            $(function () {
...
(function (e) {
    ...
    g.prototype = {
        killerFn: null, initialize: function () {
            ...
            a.suggestionsContainer = g.utils.createNode('<div class="' + d.containerClass + '" style="position: absolute; display: block;"></div>');
        ...
        suggest: function () {
            if (0 === this.suggestions.length) this.hide(); else {
                var a = this.options.formatResult, b = this.getQuery(this.currentValue),
                    c = this.classes.suggestion,
                    d = this.classes.selected, f = e(this.suggestionsContainer), g = "";
